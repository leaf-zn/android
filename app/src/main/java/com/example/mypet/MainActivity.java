...
public class MainActivity extends AppCompatActivity {
    ...
    public void initialView() {
        // 更新控件的 ID
        bn_createPet = findViewById(R.id.btnCreatePet);
        bn_backPet = findViewById(R.id.btnRecoverPet);
        bn_showPet = findViewById(R.id.btnReleasePet);
        bn_delete = findViewById(R.id.btnDeletePet);

        petAgeView = findViewById(R.id.tvPetAgeDisplay); // 更新为新的宠物年龄显示控件 ID
        petMode = findViewById(R.id.modeList);         // 宠物类型选择的 Spinner
        editName = findViewById(R.id.etPetName);        // 输入宠物名称
        petNameList = findViewById(R.id.spPetType);     // 宠物名称下拉列表（如果有）
        showPetMode = findViewById(R.id.tvPetTypeDisplay); // 显示宠物类型
        showPetName = findViewById(R.id.tvPetNameDisplay); // 显示宠物名称
        showPetImg = findViewById(R.id.ivPetImage);     // 显示宠物图片

        // 提取数据库加载到 petNameList（如果是一个新的 Spinner）
        pet_list = getAllPetInfo();

        // 创建适配器，将宠物名称加载到 Spinner 中
        petNameAdapter = new ArrayAdapter<>(this, android.R.layout.simple_spinner_item);
    ...
    public void exeListen() {
        ...
        // 显示宠物年龄
        long petAge = getPetAge(petName); // 或其他宠物名称变量
        if (petAge == 0) {
            petAgeView.setText("宠物年龄：未设置");
        } else {
    ...
    public void removePet(String name) {
        ...
        if (petNameAdapter.getCount() == 0) {
            ...
            // 显示宠物图像
            showPetImg.setImageResource(R.drawable.sample_pet);
            // 显示宠物年龄
            petAgeView.setText("");
        }
    }
